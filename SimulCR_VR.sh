...
echo -e '\nGenerates sequences under the same evolutionary model of HEP and LEP proteins both on higly and lowly expressed proteins.\n\n'
...
ls -d */*/*/ | grep 'CC' -v | grep 'PHE' -v | while read -r D; do
	...
	if [ ! -f "modeltest.out" ]; then
		#generating nt.ali
		input=$(ls *.aa_ali.fasta | grep 'sym' -v )
		...
		#generating nt.ali
		input=$(ls *.nt_ali.fasta | grep 'sym' -v)
		ALNL=$(sed -n '2p' ${input} | wc -m)
		ALNL=$(echo $((${ALNL} - 4)))
		ALNS=$(grep -v '>' ${input} | wc -l)
		touch nt.phy
	...
	if [ ! -f "mc.paml" ]; then
		if [[ $(command -v evolver) ]]
			then 
			echo -e "Generating symulated dataset."
		...
		echo "Symulation already run."
	fi


	#Extract C and VR
	if [[ ! -f "${protID}.sym.aa100C.fas" && -f "mc.paml" ]]; then
		echo "Extracting CR and VR for the symulated data."
		#phy to fasta
		tail -n +5 mc.paml | sed -E 's/^([^ ]*)  />\1\n/' | sed -e 's/ //g' > ${protID}.sym #nombre del archivo
		AD=$(pwd)
		cd "$WD"
		./translatorx_vLocal.pl -i "${AD}/${protID}.sym" -o "${AD}/${protID}.sym" &> /dev/null
		#Generates all the aa_ali.fasta and nt_ali.fasta
		cd "$AD"
		
		#removing enters from alignments
		cat "./${protID}.sym.nt_ali.fasta" | perl -pe 'unless(/^>/){s/\n//g};s/>/\n>/' | sed '/^$/d' > "./${protID}.sym.nt_ali.fastaed"
		cat "./${protID}.sym.aa_ali.fasta" | perl -pe 'unless(/^>/){s/\n//g};s/>/\n>/' | sed '/^$/d' > "./${protID}.sym.aa_ali.fastaed" 
		mv "./${protID}.sym.nt_ali.fastaed" "./${protID}.sym.nt_ali.fasta"
		mv "./${protID}.sym.aa_ali.fastaed" "./${protID}.sym.aa_ali.fasta"
		
		#extracting cr and vr
		cd "$WD"
		./codonAlnStats.pl -i "${AD}/${protID}.sym.nt_ali.fasta" -a "${AD}/${protID}.sym.aa_ali.fasta" -o "${AD}/${protID}.sym" &> /dev/null
		Rscript --vanilla stats.R "${AD}/${protID}.sym.aa.table" "${AD}/${protID}.sym.nt.table" "${AD}/${protID}.sym" "${VI}" "${protID}" &> /dev/null
