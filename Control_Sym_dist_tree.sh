...
{
echo "
        Usage: Control_Sym_dist_tree [-h]]

        -h Show this Help

"

echo -e '\nCalculate modal sequences for symulated data and generates a NJ distances tree of CUF for Ci, Singletons, expression sets and symulated data.\n\n'
...
ls -d */ | while read D; do
	...
	echo -e "Generating multifasta and modal sequence of symulated Conserved/variable regions of both HEP and LEP. \n"
	# Calculates modals and concatenated sequences for CR VR and CR VR
	ls -d */ | grep 'CC' -v | grep 'PHE' -v | while read ED; do
		EFN=$(echo "${ED}" | sed 's/\/$//')
		echo -e "\nProcessing ${EFN}.\n"
		#CR
		cat $(ls ${ED}*/* | grep 'sym.aa100C.fas$') > "${SN}_${EFN}_sym_CR.fas"
		#VR
		cat $(ls ${ED}*/* | grep 'aaV.fas$') > "${SN}_${EFN}_sym_VR.fas"
		#calculate modals and modal seqs. for CR and VR
		compile_codon_counts  < "${SN}_${EFN}_sym_CR.fas" | modal_usage_from_counts -a $(nproc) > "${SN}_${EFN}_sym_CR.modal_freq"
		compile_codon_counts  < "${SN}_${EFN}_sym_VR.fas" | modal_usage_from_counts -a $(nproc) > "${SN}_${EFN}_sym_VR.modal_freq"
		cd ..
		perl seq2AvgAA.pl "${D}${SN}_${EFN}_sym_CR.fas"
		echo -e "\n"
		perl seq2AvgAA.pl "${D}${SN}_${EFN}_sym_VR.fas"
		echo -e "\n"
		perl modal2seq2.pl "${D}${SN}_${EFN}_sym_CR.modal_freq" "${D}${SN}_${EFN}_sym_CR.aaav"
		perl modal2seq2.pl "${D}${SN}_${EFN}_sym_VR.modal_freq" "${D}${SN}_${EFN}_sym_VR.aaav"
		sed 's/>.*/>'${EFN}'_sym_CR/' "${D}${SN}_${EFN}_sym_CR.modal_freq.fas" > "${D}${SN}_${EFN}_sym_CR.modal_freq.fased"
		sed 's/>.*/>'${EFN}'_sym_VR/' "${D}${SN}_${EFN}_sym_VR.modal_freq.fas" > "${D}${SN}_${EFN}_sym_VR.modal_freq.fased"
		mv "${D}${SN}_${EFN}_sym_CR.modal_freq.fased" "${D}${SN}_${EFN}_sym_CR.modal_freq.fas"
		mv "${D}${SN}_${EFN}_sym_VR.modal_freq.fased" "${D}${SN}_${EFN}_sym_VR.modal_freq.fas"
		cd "${D}"
	done
	cd ..
done
...
#Generate symulated HEP and LEP modals

ls -d */ | while read D; do
	cd "${D}"
	ls -d */ | grep 'CC' -v | grep 'PHE' -v | while read E; do
		cd "${E}"
		file="$(echo ${E} | sed -E 's/\/$//')_sym"
		if [ ! -f "$file.fas" ]; then 
			touch "$file.fas"
		else
			rm "$file.fas"
		...
		ls -d */ | while read P; do
			...
			grep -A 1 "${protID}" ${P}${protID}.sym >> $file.fas
		done
		cd ../../
...
ls -d */ | while read D; do
	cd "$D"
	#calculate NJ distances tree
	ls *.modal_freq | grep 'sym' | sort -V | sed -e 's/.modal_freq$//' -e 's/[^_]*_//' > CCindex2.txt
	...
	cat $(ls *.modal_freq | grep 'sym' | sort -V) $(ls */*.modal_freq | sort -V | grep '[Pp][Hh[Ee]' -v  | grep '[Gg][Nn[Mm]' -v | grep 'HEP.m' -v | grep 'LEP.m' -v ) > fmdata22.txt
